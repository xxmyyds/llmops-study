...
import dotenv
from langchain_core.messages import ToolCall, AIMessage, HumanMessage, ToolMessage
...
# 1.创建大语言模型并绑定工具
llm = ChatOpenAI(model='deepseek-chat', temperature=0).bind_tools([complex_tool])
better_llm = ChatOpenAI(model='deepseek-v4-pro', temperature=0).bind_tools([complex_tool])
# 2.创建链并执行工具
...
# 2.创建大语言模型并绑定工具
llm = ChatOpenAI(model="deepseek-chat", temperature=0).bind_tools(
